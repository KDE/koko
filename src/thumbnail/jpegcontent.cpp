...
Orientation JpegContent::orientation() const
{
    ...
    if (it == d->mExifData.end() || it->count() == 0 || it->typeId() != Exiv2::unsignedShort) {
        return Orientation::NOT_AVAILABLE;
    }
...
void JpegContent::resetOrientation()
{
    ...
    *it = uint16_t(Orientation::NORMAL);
}
...
struct OrientationInfo {
    OrientationInfo()
        : orientation(Orientation::NOT_AVAILABLE)
        , jxform(JXFORM_NONE)
    {
    }
...
static const OrientationInfoList &orientationInfoList()
{
    ...
    if (list.size() == 0) {
        ...
        list << OrientationInfo() << OrientationInfo(Orientation::NORMAL, QTransform(), JXFORM_NONE)
             << OrientationInfo(Orientation::HFLIP, hflip, JXFORM_FLIP_H) << OrientationInfo(Orientation::ROT_180, createRotMatrix(180), JXFORM_ROT_180)
             << OrientationInfo(Orientation::VFLIP, vflip, JXFORM_FLIP_V) << OrientationInfo(Orientation::TRANSPOSE, hflip * rot90, JXFORM_TRANSPOSE)
             << OrientationInfo(Orientation::ROT_90, rot90, JXFORM_ROT_90) << OrientationInfo(Orientation::TRANSVERSE, vflip * rot90, JXFORM_TRANSVERSE)
             << OrientationInfo(Orientation::ROT_270, createRotMatrix(270), JXFORM_ROT_270);
    }
    return list;
}
...
{
    if (orientation != Orientation::NOT_AVAILABLE && orientation != Orientation::NORMAL) {
...
QImage JpegContent::thumbnail() const
{
    ...
    if (!d->mExifData.empty()) {
        ...
        if (o != Orientation::NORMAL && o != Orientation::NOT_AVAILABLE) {
