...
static const ThumbnailGroup s_thumbnailGroups[] = {
...
static QString generateThumbnailPath(const QString &uri, ThumbnailGroup group)
{
...
QString ThumbnailProvider::thumbnailBaseDir(ThumbnailGroup group)
{
...
static void moveThumbnailHelper(const QString &oldUri, const QString &newUri, ThumbnailGroup group)
{
...
void ThumbnailProvider::setThumbnailGroup(ThumbnailGroup group)
{
...
QImage ThumbnailProvider::loadThumbnailFromCache() const
{
    ...
    int largeThumbnailGroup = static_cast<int>(mThumbnailGroup);
    while (image.isNull() && ++largeThumbnailGroup <= static_cast<int>(ThumbnailGroup::XXLarge)) {
        // If there is a large-sized thumbnail, generate the small-sized version from it
        const QString largeThumbnailPath = generateThumbnailPath(mOriginalUri, static_cast<ThumbnailGroup>(largeThumbnailGroup));
        const QImage largeImage(largeThumbnailPath);
        if (!largeImage.isNull()) {
            const int size = pixelSize(mThumbnailGroup);
...
void ThumbnailProvider::checkThumbnail()
{
    ...
    if (MimeTypeUtils::fileItemKind(mCurrentItem) == MimeTypeUtils::KIND_RASTER_IMAGE) {
        ...
        const int _pixelSize = pixelSize(mThumbnailGroup);
        if (mPreviewPlugins.isEmpty()) {
            mPreviewPlugins = KIO::PreviewJob::availablePlugins();
        }
        KIO::Job *job = KIO::filePreview(list, QSize(_pixelSize, _pixelSize), &mPreviewPlugins);
        // KJobWidgets::setWindow(job, qApp->activeWindow());
