...
Kirigami.Page {
    ...
    Connections {
        target: mediaViewFilterModel

        function onModelReset() { root.updateModelReady(); }
        function onRowsInserted() { root.updateModelReady(); }
        function onRowsRemoved() { root.updateModelReady(); }
        function onDataChanged() { root.updateModelReady(); }
    }
    ...
    title: {
        const urlParts = root.url.toString().split("/");
    ...
    actions: [
        ...
        Kirigami.Action {
            ...
            onTriggered: Koko.PrinterHelper.printFileFromUrl(listView.currentItem.url, root.Window.window)
        },
        Kirigami.Action {
            displayHint: Kirigami.DisplayHint.AlwaysHide
            separator: true
    ...
    ListView {
        ...
        onCurrentItemChanged: {
            if (currentItem) {
                ...
                root.title = Koko.DirModelUtils.fileNameOfUrl(currentItem.url);
            }
        }
